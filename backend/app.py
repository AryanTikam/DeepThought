...
def analyze_fiction_api():
    ...
    if folder_path:
        ...
        if not os.path.exists(folder_path) or not os.path.isdir(folder_path):
            return jsonify(
                {
                    "error": f"The specified folder '{folder_path}' does not exist or is not a directory"
                }
            ), 400
    ...
    if not files or all(getattr(f, "filename", "") == "" for f in files):
        return jsonify(
            {
                "error": "No file part, no selected files, or no valid .txt files in the specified folder"
            }
        ), 400

    combined_text_content = (
        io.StringIO()
    )  # Use StringIO to build the combined text efficiently
    processed_filenames = []
    total_size = 0
        ...
        if file and getattr(file, "filename", "") and allowed_file(file.filename):
            ...
            try:
                ...
                if total_size > app.config["MAX_CONTENT_LENGTH"]:
                    return jsonify(
                        {
                            "error": f"Combined file size exceeds limit of {app.config['MAX_CONTENT_LENGTH'] // (1024 * 1024)} MB."
                        }
    ...
    if not processed_filenames:
        return jsonify(
            {"error": "No valid .txt files were provided or found in the request."}
        ), 400
    if not final_text_content.strip():
        return jsonify(
            {
                "error": "The provided .txt files were empty or contained only whitespace."
            }
        ), 400

    print(
        f"Starting analysis for combined content from: {', '.join(processed_filenames)}..."
    )
    ...
    try:
        ...
        traceback.print_exc()
        return jsonify(
            {"error": f"An unexpected server error occurred during analysis: {str(e)}"}
        ), 500
...
def home():
    """Basic welcome route"""
    return (
        "Fictional Universe Consistency Kit API is running. POST one or more .txt files to /analyze or specify a folder with .txt files using 'folder' parameter.",
        200,
    )
