...
def upload_file():
    ...
    print("universe id is: ", universe_id)
    if not universe_id:
        return jsonify({"error": "Universe ID is required"}), 400

    # universe_name = universe_id.replace("-", "_")
    universe_path = os.path.join(UNIVERSES_DIR, universe_id)
...
def delete_file():
    ...
    # universe_name = universe_id.replace("universe-", "")
    file_path = os.path.join(UNIVERSES_DIR, universe_id, filename)

    if os.path.exists(file_path):
        try:
            os.remove(file_path)
            # Re-analyze the universe after file deletion
            analyze_universe(os.path.join(UNIVERSES_DIR, universe_id))
            return jsonify({"success": True})
        except Exception as e:
            return jsonify({"error": str(e)}), 500
...
def get_knowledge_graph():
    ...
    return jsonify({"knowledge_graph": knowledge_graph, "graph_image": graph_image})
    ...
    return jsonify(
        {
            ...
            "speculation_boundaries": universe_analysis[universe_id].get(
...
def generate_graph_image(knowledge_graph):
    ...
    if (
        not knowledge_graph
        or "nodes" not in knowledge_graph
        or "edges" not in knowledge_graph
    ):
        return None
    ...
    for edge in knowledge_graph["edges"]:
        if edge["source"] in node_labels and edge["target"] in node_labels:
            G.add_edge(
                edge["source"], edge["target"], label=edge.get("relationship", "")
            )
    ...
    buffer = io.BytesIO()
    plt.savefig(buffer, format="png", bbox_inches="tight", dpi=100, facecolor="#fcfcff")
    plt.close()
...
def analyze_universe(universe_path):
    ...
    try:
        ...
        response = requests.post(
            "http://localhost:8000/analyze", files={"folder": (None, universe_path)}
        )
